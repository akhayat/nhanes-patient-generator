...
def get_tables(cursor, table_suffix):
    if table_suffix == None:
        cursor.execute(db_tool.query('tables_null_suffix').format(db_name=sql.Literal(config('DB_NAME'))))
    else:
        cursor.execute(db_tool.query('tables').format(db_name=sql.Literal(config('DB_NAME'))), [('%' + table_suffix) if table_suffix else '']), 
...
def get_all_data_for_seqn(cursor, table_list, seqn):
    ...
    for table in table_list:
        ...
        if cursor.rowcount > 0:
            ...
            cursor.execute(db_tool.query('sas_labels'), [table_name])
